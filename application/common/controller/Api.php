...
//        }
    ...
    private function run(){
        ...
        $timestamp = Request::post('timestamp',''); // 暂时不检查时间戳
        $user_id = Request::post('user_id','');
        $sign = Request::post('sign');

        //
        $secret = Env::get('SECRET_KEY');
        $checkStr = $data . $nonce . $secret . $timestamp . $user_id;
        Log::write($data);
        Log::write($checkStr);
        Log::write($sign);//
        $checkSign = strtoupper( md5($checkStr) );
        Log::write($checkSign);
        Log::write($checkSign === $sign);
        //
        if( $checkSign === $sign ){
            Log::write(111);
        }else{
            Log::write(222);
            $this->apiReturn(Code::SIGN_ERROR,'验签错误！');
            exit();
        }
