...
//            $this->error('请登录后再试', 'admin/login/login');
        ...
        $this->settings = Setting::whole();//获取配置项
        ...
        //菜单机制
        $menuList = cache($admin['role_id'] . 'menu');
        if (!$menuList) {
            $userRolesArray = $admin['rules'];
            if(is_array($userRolesArray))
            {
                $map1[] = ['id', 'in', $userRolesArray];
            }
            $map1[] = ['is_menu', '=', 1];
            $map1[] = ['is_show', '=', 1];
            $map1[] = ['status', '=', 1];
            ...
            //菜单写入缓存
            cache($admin['role_id'] . 'menu', $menuList);
        }
        //导航栏
        $navList = cache('nav');
        if (!$navList) {
    ...
    public function isSuperAdmin()
    {
        if ($this->isLogin()) {
            ...
            $admin_group_id = model('admin/Admin')->where('id','eq',$admin_user['uid'])->field('group_id')->find();

            if($admin_group_id){
                $group_info = model('admin/AdminAuthGroup')->where('id','eq',$admin_group_id['group_id'])->find();
                if($group_info['pid'] == 1 || $group_info['id'] == 1){//超级管理员
                    $is_super_admin = true;
                }
            }
