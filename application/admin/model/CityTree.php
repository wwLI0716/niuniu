...
class CityTree extends Model
{
    protected $table = 'ecshecom_city';
    // 开启自动写入时间戳字段
    ...
    {    
        $data = $this->where($map)->column('name', 'id');
        return $data;
    }
    ...
    {
        $list = $this->where('status', 1)->column('name', 'id');
        return $list;
    }
    ...
    private function _toFormatAuthTree($list, $level = 0, $title = 'name')
    {
    ...
    public function toFormatAuthTree($list, $title = 'name', $pk = 'id', $pid = 'parent_id', $root = 0)
    {
