...
class NewTree extends model
{
    protected $table = 'ecshecom_portal_category';
    // 开启自动写入时间戳字段
    ...
    {
        $data = $this->where('status', 1)->column('zh_tw_name', 'id');
        return $data;
    }
    ...
    {
        $list = $this->where('status', 1)->column('zh_tw_name', 'id');
        return $list;
    }
    ...
    private function _toFormatAuthTree($list, $level = 0, $title = 'zh_tw_name')
    {
    ...
    public function toFormatTree($list, $title = 'zh_tw_name', $pk = 'id', $pid = 'parent_id', $root = 0)
    {
    ...
    private function _toFormatTree($list, $level = 0, $title = 'zh_tw_name')
    {
        foreach ($list as $key => $val) {
    ...
    public function toFormatAuthTree($list, $title = 'zh_tw_name', $pk = 'id', $pid = 'parent_id', $root = 0)
    {
