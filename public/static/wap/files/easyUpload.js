    ...
    this.each(function (index, element) {
      ...
      var easyManager = {
        init: function () {
          ...
          $html += 'class="fileInput" data-count="0" style="display:none;" multiple accept="images/*" />';
          var ua=navigator.userAgent.toLowerCase();
          var tag=ua.match(/iPhone\sOS/i) == "iphone os";
          if (tag) {
              $html += '<span class="easy_upload_select noselect" data-tag="1">' + option.selectText + '</span>';
          } else {
              $html += '<span class="easy_upload_select noselect" data-tag="2">拍照上传</span>';
              $html += '<span class="easy_upload_select noselect" data-tag="3">选择文件</span>';
          }
          
          $html += option.multi ? '<span class="easy_upload_head_btn1 noselect" style="display:none;">上传</span>' : '';
          $html += option.multi ? '<span class="easy_upload_head_btn2 noselect" style="display:none;">删除</span>' : '';
          $html += option.multi ? '<i class="easyUploadIcon noselect head_check" data-checked="no" style="display:none;">&#xe693;</i>' : '';
          $html += option.showNote ? '<span class="easy_upload_note">' + option.note + '</span>' : '';
          $html += '</div>';
          $html += '<ul class="easy_upload_queue" id="all_item"></ul>';
          $html += '</div>';
          _ele.html($html);
          this.bindHead();
        },
        bindHead: function () {
          ...
          $('.easy_upload_select').off('click').click(function () {
            if($(this).data("tag")==2){
              $(this).parent().find('.fileInput').attr("capture","camera");
              $(this).parent().find('.fileInput').attr("accept","image/*");
            }
            if($(this).data("tag")==3){
              $(this).parent().find('.fileInput').removeAttr("capture");
              $(this).parent().find('.fileInput').attr("accept","*");
            } 
            $(this).parent().find('.fileInput').trigger('click');
          });
          $('.fileInput').off('change').change(function (e) {
            e.preventDefault();
            let files;
            if (e.dataTransfer) {
        ...
        _renderFile: function (fileArr, target) {
          ...
          this.bindQueue();
          // var self=this;
          // self._uploadFile($("#all_item"));
          // // $("#all_item li").each(function () {
          // //     self._uploadFile($(this));
          // // });

          var queueUl = $('#all_item');
          var arr = this._findItems(1, queueUl);
          if (arr.length>0) {
            allowFiles = allowFiles.concat(arr);
            upFiniehed = true;
            this._uploadFile(queueUl);}
        },
