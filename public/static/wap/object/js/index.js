...
function modelConfirm() {
    ...
    $('#shortcutEnter').click(function () {
        ...
        function init_hasShortcut_html() {
            ...
            var select_iHref_data = [];
            //选中的快捷方式的id
            var select_iModuleId_data = [];
            /*清空数组，防止遍历后重复添加*/
            var shortcutSelected_data = [];
            var has_p_data =[];                                       //快捷面板上已有的元素文字
            var has_iclass_data =[];                                 //快捷面板上已有的元素类
            var has_iHref_data =[];                                 //快捷面板上已有的元素类
            var has_iModuleId_data =[];
            //快捷面板上的快捷内容
            ...
            $.each(acticveSelect,function (i) {
                //赋值数组
                select_iClass_data.push(acticveSelect.eq(i).find('img').attr('src'));
                select_spanText_data.push(acticveSelect.eq(i).find('span').text());
                select_iHref_data.push(acticveSelect.eq(i).find('a').attr('href'));
                select_iModuleId_data.push(acticveSelect.eq(i).find('a').attr('data-module_id'));
            });
            module_ids = select_iModuleId_data;
            //遍历快捷面板上已有的元素
            $.each($('.shortcut-box li:not(:last)'),function (i) {
                has_p_data.push($('.shortcut-box li:not(:last)').find('p').text());
            });
            $.each($('.shortcut-box li:not(:last)'),function (i) {
                has_iclass_data.push($('.shortcut-box li:not(:last)').find('img').attr('src'));
            });
            $.each($('.shortcut-box li:not(:last)'),function (i) {
                has_iHref_data.push($('.shortcut-box li:not(:last)').find('a').attr('href'));
            });
            $.each($('.shortcut-box li:not(:last)'),function (i) {
                has_iModuleId_data.push($('.shortcut-box li:not(:last)').find('a').attr('data-module_id'));
            });
            ...
            for(var i=0; i<test(has_iclass_data,select_iClass_data,select_iHref_data).length; i++){
                var select_i_class = test(has_iclass_data,select_iClass_data)[i];
                var select_i_span = test(has_p_data,select_spanText_data)[i];
                var select_i_href = test(has_iHref_data,select_iHref_data)[i];
                var select_i_moduleId = test(has_iModuleId_data,select_iModuleId_data)[i];
                var select_person ={"iClass":select_i_class, 'spanText':select_i_span,'iHref':select_i_href, 'iModuleId':select_i_moduleId};
            ...
            for(var j =0; j<shortcutSelected_data.length; j++){
                ...
                hasShortcut_html += '<a href="'+hasObj.iHref+'">';
                hasShortcut_html += '<img src="'+hasObj.iClass+'">';
                hasShortcut_html += '<span>'+hasObj.spanText+'</span>';
                hasShortcut_html += '</a>';
                hasShortcut_html += '</li>';
            }
        ...
        $.post("/Wap/Site/addEntranceDefined", {module_ids: module_ids}, function (res) {
            if (res.code != 0) {
                layer.msg(res.msg);
            }
        }, 'json')
