...
if($_REQUEST['ajaxbool']=="true")
{
	echo json_encode(array("status"=>1,"path"=>$GLOBALS["sPathFileName"]));
	die();
}
else
...
if($_REQUEST['ajaxbool']=="true")
{
	if ($_FILES['uploadfile']['size']>5000*1024) {
		echo json_encode(array("status"=>0,"msg"=>"上传附件大小不能超过5000kb!"));
		die();
	}
}
...
$sWSRootUrl = GetSafeUrl(TrimGet("ws"));
$s_SKey = TrimGet("skey");
...
$s_PortSecure = isset($_SERVER["SERVER_PORT_SECURE"])?$_SERVER["SERVER_PORT_SECURE"]:"0";
$s_Protocol = "http://";
if ($s_PortSecure == "1"){
$s_Protocol = "https://";
}
