                        ...
                        <div class="col-lg-12">
                            <div class="table-container show-module-content">
                                <form id="examineInfo" name="examineInfo" action="<?php echo url('getreleaseExamine'); ?>" method="post">
                                    ...
                                    <input type="hidden" id="<?php echo htmlentities($statusName); ?>" name="<?php echo htmlentities($statusName); ?>" value="0">
                                    ...
                                    <div class="row special-row">
                                        <textarea class="form-control" name="<?php echo htmlentities($reasonName); ?>" id="<?php echo htmlentities($reasonName); ?>" rows="5"><?php if($examineInfo[$reasonName] == ''): ?>请输入审核意见<?php else: ?><?php echo htmlentities($examineInfo[$reasonName]); endif; ?></textarea>
                                    </div>
                                    <?php if($commonExamineStatus == 0): ?>
...
$(function () {
    ...
    $('.toUrl').click(function(){
        if($(this).val() == "同意")
        {
            $("#<?php echo htmlentities($statusName); ?>").val(1);
        } else {
            $("#<?php echo htmlentities($statusName); ?>").val(0);
        }
        $("#examineInfo").submit(function(){
            ...
            layer.open({
                ...
                ,yes: function(index){
                    ...
                    //1.发送数据到服务器
                    $.post("<?php echo url('Examine/getCommonExamine'); ?>",forthat.serialize(),function(res){
                        var res = JSON.parse(res);
                        layer.closeAll();
